  ...
  methods: {
    upload($event) {
      ...
      files.forEach((file) => {
        ...
        const storageRef = storage.ref(); // musci-c223.appspot.com
        const songsRef = storageRef.child(`songs/${file.name}`); // musci-c223.appspot.com/songs/example.mp3
        ...
        task.on('state_changed', (sanapshot) => {
          const progress = (sanapshot.bytesTransferred / sanapshot.totalBytes) * 100;
